...
import { Bell, BellOff, Settings2, Sparkles, Info } from "lucide-react";
...
export function NotificationSettings() {
  ...
  const { preferences, updating, updatePreference } =
      ...
      <CardHeader className="bg-muted/20 border-b pb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <CardDescription>
              Stay updated on claims and connections.
            </CardDescription>
          </div>
        </div>
      </CardHeader>
      <CardContent className="p-6 space-y-6">
        <div className="flex items-start justify-between gap-4">
          <div className="space-y-1">
            ...
            <p className="text-xs text-muted-foreground leading-relaxed">
              Receive alerts when someone claims an item from your wishlist or
          ...
          <div className="bg-destructive/10 border border-destructive/20 rounded-xl p-4 flex items-start gap-3">
            ...
            <div className="space-y-1">
              ...
              <p className="text-[11px] text-destructive/80 leading-tight">
                Your browser has blocked notification permissions. Please enable
                them in your browser settings to receive updates.
              </p>
            </div>
          </div>
        )}
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div className="space-y-0.5">
                  ...
                  <p className="text-[10px] text-muted-foreground italic">
                    Notify when someone claims a gift
                  </p>
                </div>
                <Switch
                  checked={preferences?.claim_notifications ?? true}
                  onCheckedChange={(checked) =>
                    updatePreference("claim_notifications", checked)
                  }
                  disabled={updating}
                />
              </div>
              <div className="flex items-center justify-between">
                ...
                <Switch
                  ...
                  onCheckedChange={(checked) =>
                    updatePreference("follow_notifications", checked)
                  }
                  disabled={updating}
                />
              </div>
              <div className="flex items-center justify-between">
                <div className="space-y-0.5">
                  <p className="text-xs font-semibold">Wishlist Shares</p>
                  <p className="text-[10px] text-muted-foreground italic">
                    Notify when someone shares a wishlist with you
                  </p>
                </div>
                <Switch
                  checked={preferences?.wishlist_share_notifications ?? true}
                  onCheckedChange={(checked) =>
                    updatePreference("wishlist_share_notifications", checked)
                  }
                  disabled={updating}
                />
              </div>
        ...
        <div className="bg-primary/10 rounded-xl p-4 flex items-start gap-3 border border-primary/20 transition-colors">
          ...
          <p className="text-[11px] text-muted-foreground leading-relaxed">
            ...
            screen even when the app is closed.
          </p>
        </div>
      </CardContent>
    </Card>
